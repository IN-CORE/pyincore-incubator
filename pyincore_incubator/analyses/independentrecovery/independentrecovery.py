...
class IndependentRecovery(BaseAnalysis):
    ...
    def run(self):
        ...
        building_damage = self.get_input_dataset("building damage").get_dataframe_from_csv(low_memory=False)
        household_inventory = self.get_input_dataset("household inventory").get_dataframe_from_csv(low_memory=False)
        household_allocation = self.get_input_dataset("household allocation").get_dataframe_from_csv(low_memory=False)
        household_dislocation = self.get_input_dataset("household dislocation").get_dataframe_from_csv(low_memory=False)
        residential_recovery = self.get_input_dataset("residential recovery").get_dataframe_from_csv(low_memory=False)
        housing_recovery = self.get_input_dataset("household housing recovery").get_dataframe_from_csv(low_memory=False)

        # Returns dataframe
        recovery_results = self.independent_recovery(building_damage, household_inventory, household_allocation,
                                                     household_dislocation, residential_recovery, housing_recovery)
        self.set_result_csv_data("housing recovery", recovery_results, result_name, "dataframe")

        return True

    def independent_recovery(self, building_damage, household_inventory, household_allocation, household_dislocation,
                             residential_recovery, housing_recovery):
        """
        ...
        merged_super = self.merge_damage_recovery_housing_building(
            building_damage, household_inventory, household_dislocation, residential_recovery,
            housing_recovery)

        modified_housing_recovery = self.modified_housing_recovery(merged_super, household_allocation)
        ...
        print("Finished independent_recovery() in " +
              str(end_start_independent_recovery - start_independent_recovery) + " secs")
    ...
    @staticmethod
    def merge_damage_recovery_housing_building(building_damage, household_inventory, household_dislocation,
                                               residential_recovery, housing_recovery):
        """
        ...
        housing_unit_inv_df = pd.merge(left=household_inventory,
                                       ...
                                       right_on='huid',
                                       how='right')
        housing_unit_inv_df = pd.merge(left=housing_unit_inv_df, right=building_damage['guid'],
                                       left_on='guid',
                                       right_on='guid',
                                       how='inner')
        housing_unit_inv_df = pd.merge(left=housing_unit_inv_df, right=housing_recovery.drop('guid', axis=1),
                                       left_on='huid',
                                       right_on='huid',
                                       how='left')
        household = pd.merge(left=housing_unit_inv_df, right=residential_recovery[['guid', 'bldgmean']],
                             left_on='guid',
                             right_on='guid',
                             how='inner')
    ...
    def modified_housing_recovery(merged_super, household_allocation):
        ...
        household_allocation = household_allocation.loc[(household_allocation['plcname10'] == 'Joplin') &
