...
class ElectricPowerNetworkRecovery(BaseAnalysis):
    ...
    def run(self):
        ...
        pole_damage_mcs_samples_df = self.get_input_dataset("pole_damage").get_dataframe_from_csv()

        self.create_graph(node_data=node_data, edge_data=edge_data)

        power_final_result_average, power_final_result_all_samples = self.recovery_analysis(
            ...
            consider_protective_devices=self.get_parameter("consider_protective_devices")
        )
    ...
    def graph_nodes_to_gdf(self, graph=None):
        ...
        for node in graph.nodes():
            ...
            attrs = {attr: graph.nodes[node][attr] for attr in graph.nodes[node] if attr != "geometry"}
    ...
    def graph_edges_to_gdf(self, graph=None):
        ...
        for attr in graph.edges[edge]:
            edges_gdf[attr] = pd.Series({edge: graph.edges[edge][attr] for edge in graph.edges()}).to_numpy()
    ...
    def create_graph(self, node_data=None, edge_data=None):
        ...
        self.node_gdf["node_type"] = self.node_gdf["node_type"].map(
            {"1": "wood", "2": "concrete", "3": "tower", "10": "substation", "20": "substation", "40": "substation"})
        ...
        for index, row in self.node_gdf.iterrows():
            self.G.add_node(row["node_id"], node_id=row["node_id"], node_type=row["node_type"],
                            guid_pole=row["guid_pole"], geometry=row["geometry"])

        # Add edges and their attributes
        for index, row in self.edge_gdf.iterrows():
            edge_attributes = {k: v for k, v in row.to_dict().items() if k != 'geometry'}
            self.G.add_edge(row["source"], row["target"], **edge_attributes, geometry=LineString(
                [self.G.nodes[row["source"]]['geometry'], self.G.nodes[row["target"]]['geometry']]))

        # assign 77 protective devices to the network
        # Filter nodes based on the required 'node_type' values
        filtered_nodes = [node for node, attr in self.G.nodes(data=True) if attr['node_type'] in ['concrete', 'wood']]
    ...
    def repair_time_power(self, road_num_pr, num_crew):
        ...
        if road_num_pr.size == 0:
            ...
            for i in range(int(np.ceil(pr.size / num_crew))):
                ...
                t_repair[i + 1, :j] = np.sort(
                    (np.maximum(5 + 2.5 * np.random.randn(j), np.zeros(j))
                     + np.maximum(4 + 2 * np.random.randn(j), np.zeros(j))) / 24
                    + t_repair[i, :j],
                    axis=0)
            ...
            Output222 = np.concatenate([Output22[:1], Output22[1:] - Output22[:-1]], axis=0)
    ...
    def is_connected_to_substation(self, graph, node):
        return any(graph.nodes[neighbor]['substation'] for neighbor in nx.neighbors(graph, node))
    ...
    def prioritize_failed_nodes(self, G_temp, node_importance_list):
        failed_nodes = [(n, d) for (n, d) in G_temp.nodes(data=True) if d['failure'] == 1]
        node_importance_array = np.array(
            [node[1]['node_id'] for node in failed_nodes if node[1]['node_id'] in node_importance_list])
        sorted_nodes = sorted(failed_nodes, key=lambda x: np.where(node_importance_array == x[1]['node_id'])[0][0])
    ...
    def restore_nodes_and_update_power(self, G_temp, prioritized_nodes, node_importance_array, time_step,
        ...
        def connected_to_substation(node):
            ...
            if node in connected_components:
                return any(G_temp.nodes[n]['node_type'] == 'substation' for n in connected_components[node])
            return False
        ...
        while any(d['failure'] == 1 for (n, d) in G_temp.nodes(data=True)):
            restore_nodes = [n for i, (n, d) in enumerate(prioritized_nodes) if
                             d['failure'] == 1 and cumulative_time[i] <= time_day]
            for n in restore_nodes:
                G_temp.nodes[n]['failure'] = 0

            G_temp_clr = nx.subgraph_view(G_temp, filter_node=lambda n: G_temp.nodes[n]['failure'] == 0)

            for restored_node in restore_nodes:
                if restored_node not in connected_components:
                    connected_component = set(nx.node_connected_component(G_temp_clr, restored_node))
    ...
    def get_building_power_back_time(self, node_failure_dict=None, node_importance_list=None, failure_link_dict=None,
                                     time_step=14):
        ...
        prioritized_nodes, node_importance_array = self.prioritize_failed_nodes(G_temp, node_importance_list)

        # restore nodes and update power attributes
        self.restore_nodes_and_update_power(G_temp, prioritized_nodes, node_importance_array, time_step,
                                            failure_link_dict)
    ...
    def get_failure_dict(self, consider_protective_devices=True):
        ...
        all_downstream_nodes = {node: set(nx.descendants(G_temp, node)) for node in G_temp.nodes()}
            ...
            while queue:
                ...
                for predecessor in G_temp.predecessors(current):
                    if predecessor not in visited:
                        if not (G_temp.nodes[predecessor].get('protective_device', False) or
                                G_temp.nodes[predecessor].get('node_type') == 'substation'):
        ...
        for node in G_temp.nodes(data=True):
            ...
            if consider_protective_devices:
                ...
                linked_nodes = downstream_nodes | upstream_nodes | additional_nodes | {node_key}
            else:
    ...
    def recovery_analysis(self, building_shapefile=None, pole_damage_mcs_samples_df=None, num_thread=1,
                          time_step=7, consider_protective_devices=True):
        ...
        power_failure_df = self.node_gdf.merge(pole_damage_mcs_samples_df, left_on='guid_pole', right_on='guid',
                                               how='left')
        ...
        power_failure_df['failure'] = power_failure_df['failure'].str.translate(translation_table)

        # Split the failure string into columns
        failure_columns = power_failure_df['failure'].str.split(',', expand=True)
        failure_columns.columns = [str(i + 1) for i in range(len(failure_columns.columns))]  # Column names start from 0

        # Concatenate the node_id and failure_columns dataframes
        power_failure_df = pd.concat([power_failure_df['node_id'], failure_columns], axis=1)
        ...
        failure_link_dict = self.get_failure_dict(consider_protective_devices=consider_protective_devices)

        # Loop through power_failure_df and save the resulting dataframes (loop through samples)
        for i in range(self.sample_num):
            sample_df = power_failure_df[['node_id', str(i + 1)]].rename(columns={str(i + 1): 'sample_failure'})
        ...
        node_id_importance_order = [self.G.nodes[node]['node_id'] for node in node_id_importance_order]

        # Run the Monte Carlo analysis
        power_final_result_gdf, power_final_result_samples = self.run_monte_carlo_analysis(
        ...
        power_final_results = gpd.GeoDataFrame(
            power_final_result_gdf.loc[:, ["guid", "geometry", "average_power_back_time"]]
        )
