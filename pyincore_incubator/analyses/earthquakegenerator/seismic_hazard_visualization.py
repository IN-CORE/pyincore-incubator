...
class SeismicHazardVisualization:
    ...
    def prepare_data(self, exceedance_probability):
        ...
        for site_id in self.setup.site_data["guid"]:
            ...
            site_im_values = [
                im
                for (id, *rest), im in self.intensity_measures.items()
                if id == site_id
            ]
    ...
    def generate_continuous_earthquake_geotiff(
        self, exceedance_probability, grid_resolution, geotiff_file_path
    ):
        """
        ...
        west, north = X.max(), Y.min()

        # I think it needs to be this, waiting for Zeinab to confirm
...
# TODO this could be included in pyincore-viz
